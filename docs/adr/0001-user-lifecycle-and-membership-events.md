...
>
> ⚠️ **状态快照**：下面各批次的"部署状态"按**记录当时**（2026-09-20）原样保留 —— 因此第 6、7 步与 P1
> 修复标注为"**未部署**"。**最终状态见文末 `## 本线状态：已关闭（2026-09-21）`**：该收尾批次已于 09-21
> 部署并冒烟通过。保留快照是为了留下演进过程，**不代表当前仍未部署**。
...
§8 七步全部实施完毕、上线并冒烟通过后，**这条线正式关闭**：

- **不再为"架构纯度"追加工作**。`member → user` 的**读**侧 3 处、`member → order` 的 3 处
  `getMonthlyStats`、`cozy-common` 拆分 —— 都**保持现状**。
  判据见本文 §1："谁拥有事实"是归属规则，**不是"必须消灭一切跨域调用"**。
  只要责任清楚、调用量稳定、有降级，跨域调用就是**可接受的设计**，不是欠债。
- **不引入告警平台**。观测只做最低闭环：`deploy/check-health.sh`（容器健康 + 三张 outbox 表的
  `DEAD`/`PENDING` 滞留 + 指标佐证），纳入**部署后固定检查**、随时可手动跑。
- **验收标准已达成**：新人券恰好一次 / 邀请新拓扑恰好一次 / outbox 最终 `SENT` / `PENDING`+`DEAD` 无异常 /
  禁用后旧 Token 立即 401 / 重放不重复发券或积分。
- **如果将来要动**：`member → order` 需**先写 ADR**（是否建 `ORDER_COMPLETED` 月度统计投影）；
  `member → user` 读侧若要消除，需先明确投影方与回填/对账方案。

> ⚠️ 记录口径：本 ADR 的"完成"指**设计中列出的步骤做完**，不是"跨域依赖全部消失"。
> 后续不要把"还有 2 条跨域边"当成未完成项。
